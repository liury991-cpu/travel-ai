...
interface TrainOption {
  trainCode: string; fromStation: string; toStation: string;
  departTime: string; arriveTime: string; duration: string; canBuy: boolean;
  seats: Record<string, string>; prices?: Record<string, number>;
}
...
const F: Record<string, number> = {
  trainNo: 2, trainCode: 3, fromCode: 6, toCode: 7, departTime: 8, arriveTime: 9, duration: 10,
  canBuy: 11, date: 13, gr: 21, rw: 23, rz: 24, tz: 25, wz: 26, yw: 28, yz: 29, ze: 30, zy: 31, swz: 32, dw: 33,
};
...
async function loadStations(): Promise<Record<string, string>> {
  ...
  for (const chunk of body.split('@')) {
    ...
    if (name && code) { map[name] = code; if (f[0]) map[f[0]] = code; }
  }
  _stations = map; return map;
}
async function getCookie(): Promise<string> {
  const res = await fetch('https://kyfw.12306.cn/otn/leftTicket/init?linktypeid=dc', { headers: HEADERS, redirect: 'manual' });
  const cookies = (res.headers.getSetCookie?.() as string[]) || [];
  return cookies.map((c) => c.split(';')[0]).join('; ');
}
async function queryTrains(fromName: string, toName: string, date: string): Promise<TrainOption[]> {
  const stations = await loadStations();
  const fromCode = stations[fromName]; const toCode = stations[toName];
  ...
  const res = await fetch(`https://kyfw.12306.cn/otn/leftTicket/query?${params}`, { headers: { ...HEADERS, Cookie: cookie } });
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    return res.status(200).json({ date: d, from, to, trains: trainsWithPrice,
      fareNote: '票价为城市间距估算（非 12306 实时价）' });
  } catch (e: any) {
